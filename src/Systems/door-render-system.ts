...
export class DoorRenderSystem implements ISystem{
    ...
    update(engine: IEngine): void {
        ...
        let door = engine.entities.findOne(DoorComponent);
            if (!door.has(RenderableComponent.name)){
                this.init(engine, door)
        }
    }
    ...
    private init(egine: IEngine, key: Entity): void{
        ...
        if (absoluteMapPosition){
            let x = absoluteMapPosition.x;
            let y = absoluteMapPosition.y - 20;
            
            key.add(this.createRenderable(AssetsConsts.doorClosed, `door${x}-${y}`, x, y));
