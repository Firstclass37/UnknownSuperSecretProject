...
export class CameraFollowSystem implements ISystem{
    ...
    update(engine: IEngine): void {
        ...
        cameraMove.offsetX = cameraComp.x +  cameraComp.width * 0.5 - (playerPos.x + settings.size.spriteWidth / 2);
        cameraMove.offsetY = cameraComp.y + cameraComp.height * 0.9 - (playerPos.y + settings.size.spriteWidth / 2);
